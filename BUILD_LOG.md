...
- **Asset universe expansion** — real, sourced data, not hand-typed: ~11,123 stock/ETF symbols from NASDAQ Trader's public symbol directory (`nasdaqlisted.txt`/`otherlisted.txt`), ~490 real cryptocurrencies from CoinGecko's public `/coins/markets` endpoint, plus hand-curated expansions for bonds/FX/real-estate (24/20/19 entries — "thousands" genuinely doesn't apply to these three classes, called out here rather than inflated). Every entry gets a deterministic seeded price/changePct via the existing `hashSymbol`/`seededRandom` machinery — still simulated data by design, same as every other asset in the app, just at ~800x the previous scale (14 → ~11,653).
...
2. **Real symbol collision**: BTC and ETH exist both as cryptocurrencies and as real NYSE-listed ETF tickers (Grayscale's Bitcoin/Ethereum trust products trade under those exact symbols), which would have silently shadowed the intended crypto assets in `find()`-by-symbol lookups. Fixed with a priority-ordered `usedSymbols` set (hand-curated first, real crypto second, generated stocks last — dropping ~120-150 colliding stock entries).
...
- **`PortfolioPanel.tsx`** — new "AI portfolio insights" section below the analytics grid: connect-prompt empty state (neither backend available) → "Get AI insights" button → loading → commentary + a **permanent, always-rendered disclaimer banner** ("Not financial advice", full explanatory body, amber warning styling with an alert-triangle icon) + source attribution (Claude Code vs. API key) + Regenerate → or an error state with Try again. The disclaimer is the app's own static translated copy, not something the model is trusted to remember to include.
